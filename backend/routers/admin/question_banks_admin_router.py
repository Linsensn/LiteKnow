...
async def admin_list_question_banks(
    user_id: Optional[int] = Query(None, description="按用户ID筛选(仅管理员可用)", examples=[8848]),  # 👇 修复：新增这个Query参数
    ...
    """管理员分页查看全局题库列表，支持按用户ID、关键字搜索和排序"""
    # 👇 修复：将 user_id 传给 target_user_id
    result = await qb_service.get_banks(
        db=db, current_user=current_admin, keyword=keyword, page=page, 
        page_size=page_size, sort_by=sort_by, is_admin_mode=True, target_user_id=user_id
    )
