...
@router.post("/batch", summary="批量新建会话")
def batch_create_sessions(
    target_user_id: int, data_list: list[session_schemas.SessionCreate], 
...
@router.put("/batch/status", summary="批量状态切换")
def batch_update_status(
    session_ids: list[int], status_data: session_schemas.SessionStatusToggle, 
    db: DBSession = Depends(get_db), admin: User = Depends(get_admin_user)  
):
...
@router.delete("/batch", summary="批量物理/逻辑删除")
...
@router.get("/statistics/task_type", summary="聚合计算：按任务类型统计")
